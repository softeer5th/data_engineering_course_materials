...
class Transform:
    ...
    # 변환 수행
    def run(self, css_selector_type, css_selector, raw_data_type, raw_data = None, droplevel = None, column_nums = [], row_dropna = False, row_drophyphen = False, column_names = [], astype = {}, reset_index = False):
        if raw_data:
            self.raw_data = raw_data
        if raw_data_type == 'json': # raw 파일 json이면 바로 리딩
            self.df = pd.read_json(self.raw_data['text'])
        elif raw_data_type == 'html': # html 이면 파싱
            ...
            print("Error: raw_data_type: Wrong raw_data_type")
            return None
        print(self.df)
...
class TransformGDP(Transform):
    ...
    # 변환 수행
    def run(self, path_region, css_selector_type, css_selector, raw_data_type, droplevel=None, column_nums=[], row_dropna=False, row_drophyphen=False, column_names=[], astype={}, reset_index=False,):
...
df = t.run(PATH_REGION, CSS_SELECTOR_TYPE, CSS_SELECTOR, RAW_DATA_TYPE,
            droplevel = None, 
            column_nums = [0, 1], 
            row_dropna= True, 
            row_drophyphen = True, 
